document.addEventListener('DOMContentLoaded', () => {
    function addProjectBox(container, title, content, likeCount, commentCount, date, username) {
    ...
    const relevanceContainer = document.getElementById('project-container-relevance');
    const latestContainer = document.getElementById('project-container-latest');

    // Add projects to relevance container
    for (let i = 0; i < 5; i++) {
        addProjectBox(relevanceContainer, 'Relevance Title ' + (i + 1), 'This is the description for relevance project ' + (i + 1), 8, 3, 'YYYY.MM.DD', 'Username');
    }

    // Add projects to latest container
    for (let i = 0; i < 5; i++) {
        addProjectBox(latestContainer, 'Latest Title ' + (i + 1), 'This is the description for latest project ' + (i + 1), 8, 3, 'YYYY.MM.DD', 'Username');
    }
